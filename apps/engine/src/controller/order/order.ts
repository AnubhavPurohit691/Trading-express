...
export function orderopen(req: Request, res: Response) {
    ...
    res.json({message:"order placed successfully",order:order,position:positionData})
}

export const orderclose=(req:Request,res:Response)=>{
    const userId=req.query.userId
    if(!userId)return ;
    const user = users.find((data)=>data.Id===userId)
    if(!user)return;
